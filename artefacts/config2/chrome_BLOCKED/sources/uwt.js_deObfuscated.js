      ...
      trackPidBase: function(e, t, i, r) {
        if (t) {
          ...
          if (s += "&tw_iframe_status=" + encodeURIComponent(u.value), u === this.IframeStatusCodes.IN_IFRAME && "" != document.referrer) {
            ...
            s += "&tw_document_referrer=" + c
          }
          var d = this.buildPixel;
          r && (s += n, s += "&cache_bust=" + Math.random(), d = this.buildScript);
          var f = this.getHiddenProp();
          this.isDocumentHidden() ? this.buildPixelWhenVizChange(s, f, d) : d(s)
        }
      },
