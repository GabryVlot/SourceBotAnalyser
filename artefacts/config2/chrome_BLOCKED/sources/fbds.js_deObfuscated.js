/*1514405329,,JIT Construction: v3549118,en_US*/
...
try {
  (function(a, b, c, d) {
    ...
    function m(v) {
      var w = [];
      for (var r = 0, s = v.length; r < s; r++) w.push(v[r][0] + "=" + encodeURIComponent(v[r][1]));
      return w.join("&")
    }

    function n(v, w) {
      var x = function x() {
        if (v.detachEvent) v.detachEvent("onload", x);
        else v.onload = null;
        w()
      };
      if (v.attachEvent) v.attachEvent("onload", x);
      else v.onload = x
    }
    ...
    h.track = function(v, w) {
      ...
      for (var r = 0, s = i.length; r < s; r++) p(i[r], v, w);
      return i.length > 0
    };
      ...
      if (x && typeof x === "object")
        for (var z in x)
          if (Object.prototype.hasOwnProperty.call(x, z)) {
            var A = x[z],
              B = A === null ? "null" : typeof A;
            if (B in {
                number: 1,
                string: 1,
                "boolean": 1
              }) y.push(["cd[" + encodeURIComponent(z) + "]", A]);
            else if (B === "object") {
              A = typeof JSON === "undefined" ? String(A) : JSON.stringify(A);
              y.push(["cd[" + encodeURIComponent(z) + "]", A])
            }
          }
      var C = m(y);
  ...
  new Image().src = "http:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m=' + encodeURIComponent('{"error":"LOAD", "extra": {"name":"' + e.name + '","line":"' + (e.lineNumber || e.line) + '","script":"' + (e.fileName || e.sourceURL || e.script) + '","stack":"' + (e.stackTrace || e.stack) + '","revision":"3549118","namespace":"FB","message":"' + e.message + '"}}');
}